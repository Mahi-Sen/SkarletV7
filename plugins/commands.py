...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        try:
            ...
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
                    text = "**🕵️ ᴊᴏɪɴ ᴛʜᴇ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ ᴛᴏ ɢᴇᴛ ꜰɪʟᴇ**"
                else:
    ...
    if data.startswith("sendfiles"):
        ...
        await k.edit("<b>ʏᴏᴜʀ ᴍᴇꜱꜱᴀɢᴇ ɪꜱ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ!!</b>")
        return
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        ...
                        InlineKeyboardButton("⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url="https://t.me/HowToVerify_xD/2")
                    ],[
                        InlineKeyboardButton('✨ ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ : ʀᴇᴍᴏᴠᴇ ᴀᴅꜱ ✨', callback_data='seeplans')
                    ]]
                    await message.reply_text(
        ...
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>ᴘʟᴇᴀꜱᴇ ꜱᴇᴀʀᴄʜ ᴀɢᴀɪɴ ɪɴ ɢʀᴏᴜᴘ</b>")
        else:
            chat_id = temp.SHORT.get(user)
        settings = await get_settings(chat_id)
        pre = 'filep' if settings['file_secure'] else 'file'
        ...
        try:
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    ...
                    await message.reply_text(
                        text="<b>📌 <u>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴠᴇʀɪꜰɪᴇᴅ ᴛᴏᴅᴀʏ, ᴘʟᴇᴀꜱᴇ ᴠᴇʀɪꜰʏ ᴀɴᴅ ɢᴇᴛ ᴜɴʟɪᴍɪᴛᴇᴅ ᴀᴄᴄᴇꜱꜱ ꜰᴏʀ ɴᴇxᴛ ꜰᴇᴡ ʜᴏᴜʀꜱ.</u> ✅\n\n♨️ ɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴅɪʀᴇᴄᴛ ꜰɪʟᴇ ᴛʜᴇɴ ʏᴏᴜ ᴄᴀɴ ʙᴜʏ ᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴘʟᴀɴꜱ ( ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪꜰʏ ) 🚀\n\n 🔥 ᴄʟɪᴄᴋ /plan ꜰᴏʀ ᴍᴏʀᴇ ᴅᴇᴛᴀɪʟꜱ 🔥 !</b>",
                        protect_content=True,
                        reply_markup=InlineKeyboardMarkup(btn)
                    )
                    return
...
async def give_premium_cmd_handler(client, message):
    ...
    if len(message.command) == 3:
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text=f"<b>ᴘʀᴇᴍɪᴜᴍ ᴀᴅᴅᴇᴅ ᴛᴏ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ꜰᴏʀ \n{time} ᴇɴᴊᴏʏ 😀\n</b>",                
            )
        else:
    ...
    if len(message.command) == 2:
      ...
      #  time = message.command[2]
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text="<b>🧑‍💻ʏᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ʀᴇᴍᴏᴠᴇᴅ ʙʏ ᴀᴅᴍɪɴꜱ ❌\n\n ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ ɪꜰ ᴛʜɪꜱ ɪꜱ ᴍɪꜱᴛᴀᴋᴇ \n\n 👮 ᴀᴅᴍɪɴ : {} \n</b>".format(OWNER_LNK),                
            )
        else:
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        await message.reply_text(f"**ʏᴏᴜʀ ᴘʟᴀɴꜱ ᴅᴇᴛᴀɪʟꜱ ᴀʀᴇ :\n\n⏰ ʀᴇᴍᴀɪɴɪɴɢ ᴛɪᴍᴇ : {remaining_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴛɪᴍᴇ + ᴅᴀᴛᴇ :  : {expiry_time}**")
    else:
        btn = [ 
        ...
        await message.reply_text(f"**😔 ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀɴʏ ᴘʀᴇᴍɪᴜᴍ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ. ɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ ᴄʟɪᴄᴋ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ./plan 👇**",reply_markup=reply_markup)
