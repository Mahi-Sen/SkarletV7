...
def get_file_id(msg: Message):
    if msg.media:
        for message_type in (
            ...
            obj = getattr(msg, message_type.value)
            if obj:
...
async def get_token(bot, userid, link):
    ...
    if MULTIPLE_SHORTNER:
        first_shortened = await get_verify_shorted_link(link, VERIFY_SHORTLINK_URL_2, VERIFY_SHORTLINK_API_2)
        final_shortened = await get_verify_shorted_link(first_shortened, VERIFY_SHORTLINK_URL, VERIFY_SHORTLINK_API)
        return str(final_shortened)
    else:
